...
def add_sparse_args(parser):
    # parser.add_argument('--sparse', type=str, default=True, help='Enable sparse mode. Default: True.')
    ...
    parser.add_argument('--theta', type=float, default=1e-5, help='upper confidence bound coefficient')
    # parser.add_argument('--theta_min', type=float, default=1e-30, help='upper confidence bound coefficient')
    parser.add_argument('--theta_decay_freq', type=float, default=400, help='theta decay frequency')
    parser.add_argument('--epsilon', type=float, default=1.0, help='upper confidence bound remainder')
    parser.add_argument('--factor', type=float, default=1.0, help='theta linear decay factor')
...
# theta cosine decay
class CosineDecayTheta(object):
    def __init__(self, theta, T_max, eta_min=1e-30, last_epoch=-1):
        print("T_max: ", T_max)
        print("eta_min: ", eta_min)
        self.sgd = optim.SGD(torch.nn.ParameterList([torch.nn.Parameter(torch.zeros(1))]), lr=theta)
        self.cosine_stepper = torch.optim.lr_scheduler.CosineAnnealingLR(self.sgd, T_max, eta_min, last_epoch)
...
class LinearDecay(object):
    def __init__(self, death_rate, factor=0.1, frequency=4000):
    ...
    def get_dr(self):
        if self.steps > 0 and self.steps % self.frequency == 0:
            return self.death_rate*self.factor
        else:
            return self.death_rate


class LinearDecayTheta(object):
    def __init__(self, theta, factor=1, theta_decay_freq=4000):
    # def __init__(self, death_rate, factor=0.99, frequency=600):
        self.theta = theta
        self.factor = factor
        self.steps = 0
        self.theta_decay_freq = theta_decay_freq

    def step(self):
        self.steps += 1

    def get_dr(self, theta, factor):
    # def get_dr(self):
        if self.steps > 0 and self.steps % self.theta_decay_freq == 0:
            return theta*self.factor
        else:
            return theta
...
class Masking(object):
    ...
    def init(self, mode='ERK', density=0.05, erk_power_scale=1.0):
        ...
        if mode == 'GMP':
            ...
            for name, mask in self.masks.items():
                ...
                print(
                    f"layer: {name}, num_parameters: {mask.numel()}, shape: {mask.shape}, density: {density_dict[name]}"
                )
        ...
        self.fired_times = copy.deepcopy(self.masks)
    ...
    def step(self):
        ...
        # theta step
        self.theta_decay.step()
        # cosine
        # self.theta = self.theta_decay.get_dr()
        # linear
        self.theta = self.theta_decay.get_dr(self.theta, self.factor)
        ...
        for module in self.modules:
            for name, tensor in module.named_parameters():
                if name in self.masks:
                    ...
                    # print("name: ", name)
                    # print("mask size: ", self.masks[name].size())
                    # print()
                    # print("masks: ", self.masks[name])
                    # reset momentum
    ...
    def truncate_weights(self):
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                ...
                # print("name: ", name)
                # print("num_remove_total: ", self.num_remove_total[name])
                # print(int(self.name2nonzeros[name] - new_mask.sum().item()))
                # self.num_remove_total[name] = self.num_remove_total[name] + int(self.name2nonzeros[name] - new_mask.sum().item())
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                ...
                # print("name: ", name)
                # print("weight: ", weight)
                # print("self.masks[name]: ", self.masks[name])
    ...
    def magnitude_death(self, mask, weight, name):
        ...
        # print("nonzeros: ", self.name2nonzeros[name])
    ...
    def pattern_magnitude_death(self, mask, weight, name):
        # TODO
        return
    ...
    def fired_masks_update(self):
        ...
        for module in self.modules:
            for name, weight in module.named_parameters():
                ...
                # print("self.masks[name].data.byte(): ", self.masks[name].data.byte())
                # print("self.fired_masks[name].data.byte(): ", self.fired_masks[name].data.byte())
                # print("self.fired_masks[name]: ", self.fired_masks[name])
