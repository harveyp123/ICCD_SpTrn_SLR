        ...
        with open(config, "r") as stream:
            try:
                ...
                for k, v in self.prune_ratios.items():
                    ...
                    # self.prune_ratios[k] = args.sparsity
...
def hard_prune_accuracy_test(ADMM, model, sparsity_type, logger, data, option=None):
    """
    hard_pruning, or direct masking
    Args:
         model: contains weight tensors in cuda
    ...
    for (name, W) in model_copy.named_parameters():
        ...
        if option == None:
            ...
            raise Exception("not implmented yet")
        W.data = cuda_pruned_weights  # replace the data field in variable
    accs = test(model_copy, data, logger)
    del model_copy
...
def z_u_update(ADMM, model, epoch, args, logger):
    ...
    if args.optimization == 'admm':
        ...
        if epoch != 1 and (epoch - 1) % args.admm_epoch == 0:
            for i, (name, W) in enumerate(model.named_parameters()):
                if name not in ADMM.prune_ratios:
                    continue
                Z_prev = None
...
def combined_masked_retrain(ADMM, model, device, data, optimizer, epoch, criterion, args, logger):
    ...
    optimizer.step()
    logger.info('re-Train Epoch: {}, Loss: {:.6f}'.format(epoch, loss.item()))
    return loss


def masked_retrain(ADMM, model, device, data, optimizer, epoch,criterion, args, logger):
