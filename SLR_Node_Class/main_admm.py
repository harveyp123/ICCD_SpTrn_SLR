...
def main():
     ...
     model = NeuralNet(args.input_size, args.hidden1_size, args.hidden2_size, args.num_classes)
    ...
    #     logger.info(i, "th weight:", name, ", shape = ", W.shape, ", weight.dtype = ", W.dtype)  
     ...
     criterion = nn.NLLLoss() #binary cross entropy
     ...
     if args.admm_train:
        ...
        for i in range(args.rho_num):
            ...
            torch.save(model.state_dict(), args.model_pruned_path + "/pruned_{}_{}_{}_Sp_{}.pt".format(current_rho/10, \
                    args.config_file, args.sparsity_type, args.sparsity))
     ...
     if args.masked_retrain:
        ...
        for epoch in range(1, args.retrain_epoch+1):
            ...
            if prec1 > (best_prec1 + 0.0015):
                ...
                torch.save(model.state_dict(),args.model_retrained_path+"/retrained_acc_{:.3f}_{}rhos_{}_{}_Sp_{}.pt".format(
                               best_prec1, args.rho_num, args.config_file, args.sparsity_type, args.sparsity))
                logger.info("\n>_ Deleting previous model file with accuracy {:.3f}% now...\n".format(old_best))
                if old_best > 0:
                    os.remove(args.model_retrained_path+"/retrained_acc_{:.3f}_{}rhos_{}_{}_Sp_{}.pt".format(
                        old_best, args.rho_num, args.config_file, args.sparsity_type, args.sparsity))
     ...
     torch.save(model.state_dict(), args.model_final_path+'/model_pruned_Sp_{}_ACC_{}.ckpt'.format(args.sparsity, \
                             prec1))
